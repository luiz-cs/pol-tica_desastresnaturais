...
saveRDS(df2009_desastres, file = here("Dados/df2009_desastres.RDS"))
```

```{r, 'pivot_colunas_dd'}
df_dd <- df2009_desastres %>% 
  select(ibge, Data, Ano, Mês, Bioma, Costeiro, Populacao_2022, IDHM, t_4a, c_4a, t_1a, c_1a, t_90, c_90, t_30, c_30, t30, c30, t90, c90, t1a, c1a, t4a,c4a)  %>% 
  pivot_longer(
  ...
  mutate("Ambiente" = paste(Bioma, Costeiro, sep = "_"),
         Mês = case_match(as.character(Mês),
                          "1" ~ "Jan", 
                          "2" ~ "Fev",
                          "3" ~ "Mar", 
                          "4" ~ "Abr",
...
reg30 <- lm(agenda_clima ~ Tratamento*Tempo + Ambiente + Mês + Populacao_2022 + IDHM, data = df_dd30)
#+ Ambiente + Mês + Populacao_2022 + IDHM
summary(reg30)
```
...
ggreg30 <- vreg30 %>% 
  ggplot(aes(x = Tempo, y=média_n_projetos, color = Tratamento, group = Tratamento))+
  geom_point(size = 5)+
  geom_line()+
...
#Fazendo a regressão:
  #Colocar multiplicação (*) entre dummy de tratamento e tempo para diff-in-diff:
reg90 <- lm(agenda_clima ~ Tratamento*Tempo + Ambiente + Mês + Populacao_2022 + IDHM, data = df_dd90)
#+ Ambiente + Mês + Populacao_2022 + IDHM
summary(reg90)
```
...
#Fazendo a regressão:
  #Colocar multiplicação (*) entre dummy de tratamento e tempo para diff-in-diff:
reg1a <- lm(agenda_clima ~ Tratamento*Tempo + Ambiente + Mês + Populacao_2022 + IDHM, data = df_dd1a)
#+ Ambiente + Mês + Populacao_2022 + IDHM
summary(reg1a)
```
...
ggreg1a <- vreg1a %>% 
  ...
  geom_vline(xintercept = 0, linetype = "dashed")+
  theme_linedraw()+
  labs(title = "Figura 9 - Diff-in-Diffs 1 ano",
        x = "Tempo em relação ao tratamento",
        y = "Nº médio de projetos climáticos")+
  scale_color_brewer(palette = "Dark2")+
  scale_x_continuous(breaks = c(-1, 0, 1),  # Set custom breaks for x-axis
...
#Fazendo a regressão:
  #Colocar multiplicação (*) entre dummy de tratamento e tempo para diff-in-diff:
reg4a <- lm(agenda_clima ~ Tratamento*Tempo + Ambiente + Mês + Populacao_2022 + IDHM, data = df_dd4a)
#+ Ambiente + Mês + Populacao_2022 + IDHM
summary(reg4a)
```
...
ggreg4a <- vreg4a %>% 
  ...
  geom_vline(xintercept = 0, linetype = "dashed")+
  theme_linedraw()+
  labs(title = "Figura 10 - Diff-in-Diffs 4 anos",
        x = "Tempo em relação ao tratamento",
        y = "Nº médio de projetos climáticos")+
  scale_color_brewer(palette = "Dark2")+
  scale_x_continuous(breaks = c(-1, 0, 1),  # Set custom breaks for x-axis
